$(function() {
	...
	$(".formPessoa input:text:first-child ").focus()
	...
	/*$(".loginPic").hover(
		...
		$('.logleft, .logback').animate({left:10, opacity:1},400); 
		$('.logright').animate({right:10, opacity:1},400); /*},
...
*/	
	...
	*/
	...
	// functra as especies pelo serviço em relatório-vistoria
	$(".servico").chosen().change(function(){
            servico = $(this).val();
            
            $.get("orcamentos/functions.php", { SER_id: servico})
            .done(function(data) {
                // limpa as opcoes
              $(".ep").empty();
              data = JSON.parse(data);
              if(data != null){
                  // adiciona as EPs filtradas
                  for(i=0; i<data.length; i++){
                      $(".ep").append('<option value="'+data[i].PRA_id+'">'+data[i].PRA_sigla+' - '+data[i].PRA_nome+'</option>')
                  }
              }else{
                  // se estiver vazio, seta uma opcao em branco
                 $(".ep").append('<option value=""></option>')
              }
              // atualiza o plugin de opcoes
              $(".ep").trigger("liszt:updated");
              

              //formacaoPreco.setValor("#mao-de-obra", valor);
            });

        });
		
                // functra as especies pelo serviço em relatório-vistoria
        $(".tratamento").chosen().change(function(){
            ep = $("#ep").val();
            ep = ep.split(' - ');
            ep = ep[0];
            ...
            $.get("orcamentos/filtragem-produto.php", { TRA_id: tratamento,ep:ep})
            .done(function(data) {
                // limpa as opcoes
              $(".pro").empty();
              data = JSON.parse(data);
              if(data != null){
                  // adiciona as EPs filtradas
                  for(i=0; i<data.length; i++){
                      $(".pro").append('<option value="'+data[i].PRO_id+'">'+data[i].PRO_sigla+' - '+data[i].PRO_nome+'</option>')
                  }
              }else{
                  // se estiver vazio, seta uma opcao em branco
	...
	$(".maskCel").mask("(99) 9999-9999?9");
	$(".maskIntPhone").mask("+55 99 999 999");
	...
	$('input[placeholder], textarea[placeholder], div[placeholder]').placeholder();
	...
	$('.tOptions').click(function() {
		...
		$(this).parent('.dyn').find('.tablePars').slideToggle(200);
		return false
	});	
	...
	$(".select, .check :checkbox, input:radio, input:file").uniform();
	$(".preco").keyup(function(){
            custo = $("#PRO_preco_custo").val();
            lucro = $("#PRO_porcent_lucro").val();
            percent = lucro/100;
            final = Number(custo)+(custo*percent);
            $(".precoFinal").val(final);
            fim = final.toString().replace('.',',');
            $("#valorPrecoFinal").html( 'R$ <b>'+fim+'</b>');
	})
	
	$(".removeAll").click(function(){
		$("input[@name='checkRow[]']:checked").animate({
			opacity: 0.25
		  }, 500, function() {
			// Animation complete.
			id = $(this).val();
	 ...
	 // permite apenas que se digite numeros em algum input  
     $(".validNum").keyup(function(){
		validNum( this );
	 });
	 
	 $("#PRO_unid_estoque").chosen().change(function(){
	 	$("#medida_produto").text($(this).val());
	 });
	...
	function preencer(json){
            for(var key in json){
                ...
                if($campo.is(":checkbox")){
                    if($campo.val() == json[key]){
                        $campo.attr("checked",true);
                    }
                }

                if($campo.is(":radio")){
                    $campo.attr("checked",false).parent('span').removeClass('checked');
                    $campo.each(function(e){
	...
	function removeLinha(linha){
		...
		calcula_material_direto();
		calcula_extras()
		var soma = 0;
		$(".demostrativo").find('.total-form-preco').each(function() {
		   soma = Math.floor(Number($(this).val())+Number(soma));
		});
		
		$("#total>b").text(soma)
		$("#FOR_total_sugerido").val(soma)
	...
	function validNum(elem){
		valor = elem.value;
		elem.value = valor.replace(/[^0-9\.]+/g,'');
	}
	
	function converteUnidadesProduto(elem){
		validNum(elem)
		
		$PRO_baixa           = $("#PRO_baixa");
		$PRO_conver_estoque  = $("#PRO_conver_estoque");
		$PRO_conver_cliente  = $("#PRO_conver_cliente");
		
		PRO_unid_cliente  = $("#PRO_unid_cliente").val();
		PRO_unid_estoque = $("#PRO_unid_estoque").val();
		
		estoque = $PRO_baixa.val()*$PRO_conver_estoque.val();
		$("#valorEstoque").html(  estoque+' '+PRO_unid_estoque );
		
		cliente = estoque*$PRO_conver_cliente.val();
		$("#valorCliente").html(  cliente+' '+PRO_unid_cliente );

		//PRO_baixa*PRO_conver_estoque=PRO_estoqueminimo*PRO_conver_cliente=PRO_cliente
		return false
	}
	...
	function check(obj){
		...
		$(".checkAll tbody tr td:first-child input:checkbox").each(function() {
			obj.checked = checkedStatus;
				if (checkedStatus == obj.checked) {
					$(obj).closest('.checker > span').removeClass('checked');
					$(obj).closest('table tbody tr').removeClass('thisRow');
				}
				if (obj.checked) {
					$(obj).closest('.checker > span').addClass('checked');
					$(obj).closest('table tbody tr').addClass('thisRow');
				}
		});	
	}
	
	function excluiLinha(id){
		// exclui a linha com opacidade
		$("#"+id).animate({
			opacity: 0.25
		  }, 500, function() {
			// Animation complete.
...
/**
 * FORMAÇÃO DE PREÇO - CLASS
 */
formacaoPreco = {
    setValor:function(campo,valor){
        $(campo).val( valor );
    }
};