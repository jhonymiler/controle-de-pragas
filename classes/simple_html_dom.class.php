...
$query = !empty($_GET['query'])? $_GET['query'] : 'officewebbrasil.com';
$pg = new simple_html_dom('http://www.whois.com/whois/'.$query);

foreach($pg->find('img[class=whois_email]') as $resultado){
    $resultado->src = 'http://www.whois.com'.$resultado->src;
}

$res = '';
foreach($pg->find('div[id=registrarData]') as $resultado){
    $res .= $resultado;
}
...
if($res){
    ...
    foreach($pg->find('div[id=avail-result]') as $resultado){
        $res .= $resultado;
    }
    echo $res;
}
